...
// Package synology contains dist Targets for building Synology Cylonix packages.
package synology
...
func (t *target) String() string {
	return fmt.Sprintf("cylonix-synology/dsm%s/%s", t.dsmVersionString(), t.filenameArch)
}
...
func (t *target) buildSPK(b *dist.Build, inner *innerPkg) ([]string, error) {
	...
	filename := fmt.Sprintf("cylonix-%s-%s-%d-dsm%s.spk", t.filenameArch, b.Version.Short, synoVersion, t.dsmVersionString())
	...
	err = writeTar(tw, b.Time,
		...
		static("Cylonix.sc", "Cylonix.sc", 0644),
		dir("conf"),
...
func (t *target) mkInfo(b *dist.Build, uncompressedSz int64) []byte {
	...
	f("displayname", "Cylonix")
	f("maintainer", "EZBLOCK, Inc.")
	f("maintainer_url", "https://github.com/cylonix/cylonix")
	f("create_time", b.Time.Format("20060102-15:04:05"))
	f("dsmuidir", "ui")
	f("dsmappname", "SYNO.SDS.Cylonix")
...
func (m *synologyBuilds) buildInnerPackage(b *dist.Build, dsmVersion int, goenv map[string]string) (*innerPkg, error) {
	...
	return m.innerPkgs.Do(key, func() (*innerPkg, error) {
		...
		err = writeTar(tw, b.Time,
			dir("bin"),
			file(tsd, "bin/cylonixd", 0755),
			file(ts, "bin/cylonix", 0755),
			dir("conf"),
			static("Cylonix.sc", "conf/Cylonix.sc", 0644),
...
func dir(name string) tarEntry {
	return func(tw *tar.Writer, modTime time.Time) error {
		return tw.WriteHeader(&tar.Header{
			...
			Mode:     0755,
			ModTime:  modTime,
			// TODO: why cylonix? Files are being written as owned by root.
			Uname: "cylonix",
			Gname: "cylonix",
		})
	}
