...
// Package qnap contains dist Targets for building QNAP Cylonix packages.
//
...
func (t *target) String() string {
	return fmt.Sprintf("cylonix-qnap/%s", t.arch)
}
...
func (t *target) buildQPKG(b *dist.Build, qnapBuilds *qnapBuilds, inner *innerPkg) ([]string, error) {
	...
	filename := fmt.Sprintf("Cylonix_%s-%s_%s.qpkg", b.Version.Short, qnapTag, t.arch)
	...
	cmd := b.Command(b.Repo, "docker", "run", "--rm",
		...
		"-v", fmt.Sprintf("%s:/cylonix", inner.tailscalePath),
		"-v", fmt.Sprintf("%s:/cylonixd", inner.tailscaledPath),
		// Cylonix folder has QNAP package setup files needed for building.
		"-v", fmt.Sprintf("%s:/Cylonix", filepath.Join(qnapBuilds.tmpDir, "files/Cylonix")),
		"-v", fmt.Sprintf("%s:/build-qpkg.sh", filepath.Join(qnapBuilds.tmpDir, "files/scripts/build-qpkg.sh")),
		"-v", fmt.Sprintf("%s:/out", b.Out),
		"build.cylonix.io/qdk:latest",
		"/build-qpkg.sh",
	)
...
func newQNAPBuilds(b *dist.Build, signer *signer) (*qnapBuilds, error) {
	...
	if signer != nil {
		...
		keyPath := filepath.Join(m.tmpDir, "files/Cylonix/private_key")
		if err := os.WriteFile(keyPath, key, 0400); err != nil {
			return nil, err
		}
		certPath := filepath.Join(m.tmpDir, "files/Cylonix/certificate")
...
func (m *qnapBuilds) buildInnerPackage(b *dist.Build, goenv map[string]string) (*innerPkg, error) {
	return m.innerPkgs.Do(goenv, func() (*innerPkg, error) {
		...
		tsPath := filepath.Join(tmpDir, "cylonix")
		if err := os.WriteFile(tsPath, tsBytes, 0755); err != nil {
			return nil, err
		}
		tsdPath := filepath.Join(tmpDir, "cylonixd")
