...
var ipCmd = &ffcli.Command{
	Name:       "ip",
	ShortUsage: "cylonix ip [-1] [-4] [-6] [peer hostname or ip address]",
	ShortHelp:  "Show Cylonix IP addresses",
	LongHelp:   "Show Cylonix IP addresses for peer. Peer defaults to the current machine.",
	Exec:       runIP,
...
func runIP(ctx context.Context, args []string) error {
	...
	if nflags > 1 {
		return errors.New("cylonix ip -1, -4, and -6 are mutually exclusive")
	}
	if !v4 && !v6 {
		v4, v6 = true, true
	}
	...
	if len(ips) == 0 {
		return fmt.Errorf("no current Cylonix IPs; state: %v", st.BackendState)
	}
	...
	if !match {
		if ipArgs.want4 {
			return errors.New("no Cylonix IPv4 address")
		}
		if ipArgs.want6 {
			return errors.New("no Cylonix IPv6 address")
		}
	}
	return nil
}
