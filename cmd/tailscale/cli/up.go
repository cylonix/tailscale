...
func newUpFlagSet(goos string, upArgs *upArgsT, cmd string) *flag.FlagSet {
	...
	upf.StringVar(&upArgs.exitNodeIP, "exit-node", "", "Cylonix exit node (IP or base name) for internet traffic, or empty string to not use an exit node")
	...
	if cmd == "up" {
		...
		upf.BoolVar(&upArgs.forceReauth, "force-reauth", false, "force reauthentication (WARNING: this will bring down the Cylonix connection and thus should not be done remotely over SSH or RDP)")
...
func updatePrefs(prefs, curPrefs *ipn.Prefs, env upCheckEnv) (simpleUp bool, justEditMP *ipn.MaskedPrefs, err error) {
	...
	if env.upArgs.forceReauth && isSSHOverTailscale() {
		if err := presentRiskToUser(riskLoseSSH, `You are connected over Cylonix; this action will result in your SSH session disconnecting.`, env.upArgs.acceptedRisks); err != nil {
			return false, nil, err
		}
	}
